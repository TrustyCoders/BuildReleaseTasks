...
[string]$Environment = Get-VstsInput -Name Environment -Require
...
#5.1. Set environment
    Write-Output "Set DOTNET_ENVIRONMENT to $Environment"
    [Environment]::SetEnvironmentVariable("DOTNET_ENVIRONMENT", $Environment, "Machine")
    Write-Output "Set ASPNETCORE_ENVIRONMENT to $Environment"
...
#5.2. Install service
    ...
    if($svc -eq $null)
    {
        New-Service -Name $ServiceName -BinaryPathName $executable -StartupType $StartType -DisplayName $DisplayName 
    }
    else {
        Set-Service -Name $ServiceName -StartupType $StartType -DisplayName $DisplayName         
    }
