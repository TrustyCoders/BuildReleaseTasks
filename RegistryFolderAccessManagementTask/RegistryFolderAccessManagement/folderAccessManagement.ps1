...
Write-Output "Adding user $Username access rights $AccessRights on registry folder $FolderName"
...
if ($AccessRightsArray.Count -gt 0){
    ...
    $AccessRightsArray | ForEach-Object {
        $accessRight=$_.Trim()
        $Ar = New-Object System.Security.AccessControl.RegistryAccessRule($Username,$accessRight,"ContainerInherit,ObjectInherit","None","Allow")
...
Write-Output "Added user $Username access rights $AccessRights on registry folder $FolderName"